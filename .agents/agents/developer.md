...
1. **How（実装方針とテストコード）の主体的設計**:
   ...
   - **Kotest マッチャーの積極活用**: アサーションには `Kotest` のマッチャー（`shouldBe`, `shouldBeEmpty` 等）を活用し、自然言語のように読める表現力豊かなテストコードを記述する。
...
2. **堅牢なコード実装と警告ゼロの徹底**:
   ...
   - コンパイラ警告（Strict Warnings）や非推奨API使用による警告を発生させないクリーンなコードを維持する。
