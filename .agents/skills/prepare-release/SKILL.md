...
> [!WARNING]
> タグ（例: `v1.0.0`）は、ユーザーがGitHubのRelease画面でリリースを作成するタイミングで付与します。AIエージェント側で `git tag` を作成したり、`git push --tags` を実行することは**絶対にしないでください**。CI（Publish workflow）が重複して実行される原因になります。
