...
## 2. 開発者体験ファースト（DX-First）と Public API の最小化

- **[人間が求める理想の実装]:** 
  - **トップダウン・DX設計**: 「内部実装がどう動くか（ボトムアップ）」ではなく、「ライブラリ利用者が最小のコードで直感的に目的を達成できるか」から逆算してAPIを設計する。
  - **高レベルAPIの提供**: 生の内部座標（`Rect` 等）を利用者に渡して自前でポップアップ座標を計算させるような低レベル設計を避け、`PopupPositionProvider` などの高レベルユーティリティを最初から提供する。
  - **情報隠蔽（Information Hiding）**: 内部専用の状態クラス（例: `WysiwygState` 等）を安易に `public` 公開しない。State Holder や高レベルコンポーネントの背後にカプセル化し、利用者に余計な管理負担を強いない。
  - **名称の自明性**: ドメインやユースケースが自明な命名を行う（例: `onTypingTrigger` のような抽象的造語ではなく、`onAutocompletion...` などユースケースが直感的に伝わる命名）。
- **[拒否されるアンチパターン]:** 
  - 利用者に低レベルの数学計算や座標変換を丸投げする粗悪なAPI設計。
  - 1つのコンポーネントを使うために複数のStateの生成・同期を強制すること。

---

## 3. Compose イディオムとジェスチャ・イベントハンドリング

- **[人間が求める理想の実装]:** 
  - **明示的イベント消費パターン**: コールバックの戻り値 `Boolean`（`true` で消費）ではなく、Compose標準（`PointerInputChange` 等）に合わせた `consume()` / `isConsumed` パターン（イベントオブジェクト自身が消費状態を持つ）を採用する。
  - **PointerInput のキー選定と不要な再起動防止**: タイピングのたびに頻繁に更新されるオブジェクト（`TextLayoutResult` やコールバックラムダ）を `pointerInput` や `remember` のキーに直接渡さない。`val currentCallback by rememberUpdatedState(callback)` や provider ラムダ（`() -> TextLayoutResult?`）を活用し、タイピングごとのコルーチンキャンセル・再起動・再アロケーションを防止する。
  - **活性・非活性（enabled）の完全な反映**: コンポーネントに `enabled` パラメータが存在する場合、描画だけでなくポインタジェスチャハンドラ（スパンタップ、クリック等）にも確実に適用し、非活性時に不要なイベントが発火しないようにする。
- **[拒否されるアンチパターン]:** 
  - 戻り値 `Boolean` による昭和的なイベント消費フラグ。
  - 毎文字タイピングするたびに `pointerInput` がキャンセル・再起動されるパフォーマンス上の欠陥。
  - `enabled = false` なのにクリックハンドラが発火してしまうバグ。

---

## 4. コンポジションと拡張性の最大化 (Open-Closed Extensibility)
...
## 5. Jetpack Compose / Compose Multiplatform パフォーマンス最適化と Opt-in 原則
...
- **[人間が求める理想の実装]:** 
  ...
  - また、正規表現によるURLパースや全文検出など、アロケーションや処理速度コストが高い処理は常時自動実行（Opt-out）させず、デフォルトOFFとする **Opt-in API 設計**（明示的なフラグや設定での有効化）を徹底する。
- **[拒否されるアンチパターン]:** 
  - Recompositionのたびに重い計算ロジックを実行したり、不安定な型（`MutableList` や生キーの `Map` など）をComposeプロパティとして露出すること。

---

## 6. 型安全（Type-Safe）な属性・ドメイン設計
...
## 7. 不変性（Immutability）とアトミックな状態更新
...
## 8. KMP（Kotlin Multiplatform）としての責務分離とプラットフォーム遮断
