...
class RichStringEditParagraphTest {
    ...
    @Test
    fun `headingLevel applied to text without trailing newline preserves valid character range`() {
        // Issue 102 reproduction test
        val text = "Hello"
        val richString = RichString(text = text)

        val actual =
            richString.edit {
                editAttributes(range = 0..text.lastIndex) {
                    headingLevel(HeadingLevel.H1)
                }
            }

        actual.spans shouldHaveSize 1
